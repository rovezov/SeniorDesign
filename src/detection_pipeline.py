...
class FaceWorker:
    ...
    def __init__(self, face_matcher: FaceMatcher, ppe_detector: PPEDetector = None, num_workers: int = 2, maxsize: int = 8, verbose: bool = False):
    ...
    def _process(self, person_id, crop):
        ...
        try:
            ...
            if face_crop is not None:
                if self._verbose:
                    print(f"[Person {person_id}] Face detected, attempting face matching...")
                name, confidence = self._matcher.match_face_image(face_crop)
                highest_confidence = confidence
                
                if name and name.lower() != "unknown":
                    if self._verbose:
                        print(f"[Person {person_id}] Face matching SUCCESS: {name} (confidence: {confidence:.4f})")
                    face_result = (name, confidence)
                else:
                    if self._verbose:
                        print(f"[Person {person_id}] Face matching FAILED: No match found (highest confidence: {confidence:.4f})")
            else:
                if self._verbose:
                    print(f"[Person {person_id}] Face detection FAILED: No face detected in crop")
        except Exception as e:
            if self._verbose:
                print(f"[Person {person_id}] Face matching FAILED: Exception - {str(e)}")
            pass
...
class PersonTracker:
    ...
    def __init__(self, face_confidence_threshold=0.25, min_votes=1, ppe_confidence_threshold=0.75, ppe_min_votes=1):
...
class DetectionPipeline:
    ...
    def __init__(self, ppe_requirements=None, min_tracking_time=0.5, edge_margin=0, verbose=False, face_confidence_threshold=0.25, ppe_confidence_threshold=0.75):
        ...
        self.face_cooldown = 0.01  # Reduced from 0.75s for faster initial detection attempts
    ...
    def _init_face_worker(self):
        """Initialize face worker with FaceMatcher and warm up models"""
        try:
            self._vprint("Initializing face detection models and warming up...")
            face_matcher = FaceMatcher()
            # Increased workers to 8 and queue to 10 for faster parallel processing
            self.face_worker = FaceWorker(face_matcher, ppe_detector=self.ppe_detector, num_workers=4, maxsize=10, verbose=self.verbose)
    ...
    def process_frame(self, frame):
        ...
        if self.face_worker is not None:
            for pid, result in self.face_worker.drain_results().items():
                ...
                if name is not None:
                    self.person_tracker.add_match(pid, name, confidence)
        ...
        for person in results:
            ...
            if (person['bbox'] is not None
                ...
                if crop is not None:
                    submitted = self.face_worker.submit(person_id, crop.copy())
                    if submitted:
